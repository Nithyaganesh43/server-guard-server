...
// Server URL from Module A
const char* serverUrl = "https://internetprotocal.onrender.com/config";
...
void sendPostRequest() {
  ...
  if (https.begin(client, serverUrl)) {
    ...
    // Constructing JSON using current SSID/Pass/IP
    String jsonBody = String("{\"deviceName\":\"Camera01\",") +
...
void setup() {
  ...
  // CHANGED: Set to HIGH (OFF) to match "024" state on boot
...
void applyStateFromCommand(String cmd) {
  ...
  for (int i = 0; i < cmd.length(); i++) {
    ...
    if (c == '0' || c == '1') {
      relayStates[0] = (c == '1');
      updateRelayState(0, relayStates[0]);
    }
    else if (c == '2' || c == '3') {
      relayStates[1] = (c == '3');
      updateRelayState(1, relayStates[1]);
    }
    else if (c == '4' || c == '5') {
      relayStates[2] = (c == '5');
      updateRelayState(2, relayStates[2]);
    }
  }
